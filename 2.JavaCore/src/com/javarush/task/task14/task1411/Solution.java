...
2. Метод main должен прекращать считывать строки с клавиатуры, как только введенная строка не совпадает с
одной из ожидаемых (user, loser, coder, proger).
3. Для каждой корректной (user, loser, coder, proger) введенной строки должен быть вызван метод doWork с
соответствующим объектом класса Person в качестве параметра.
    ...
    public static void main(String[] args) throws Exception {
        ...
        }
        {
            ...
            for (String next: list) {
                if (next.equals("user")) doWork(new Person.User());
                if (next.equals("loser")) doWork(new Person.Loser());
                if (next.equals("coder")) doWork(new Person.Coder());
                if (next.equals("proger")) doWork(new Person.Proger());
            }
